...
import { NotesSection } from "@/app/admin/users/[id]/NotesSection";
import { TrueSignalCard } from "@/app/admin/users/[id]/TrueSignalCard";
...
const VIDEO_TITLE: Record<string, string> = {
  ...
  "1185862328": "אתגר — שיעור פתיחה",
  "1146491419": "אתגר — יום 1: בהירות",
  "1149821176": "אתגר — יום 2: הלקוח האידיאלי",
  "1146553292": "אתגר — יום 3: המסר",
  "1147280995": "אתגר — יום 4: הסיפור",
  "1147281285": "אתגר — יום 5: תוכן",
  "1147597114": "אתגר — יום 6: הצגה",
  "1147325993": "אתגר — יום 7: סיכום",
};
...
export default async function AdminClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch email logs (table not in generated types — use safeFrom)
  const emailLogsRes = await safeFrom(supabase, "email_logs")
    .select("id, sequence_id, status, created_at")
    .eq("user_id", id)
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const sequenceIds = emailLogs.map((l: any) => l.sequence_id).filter(Boolean);
  const seqRes = sequenceIds.length
    ? await supabase
        .from("email_sequences")
        .select("id, template_key, subject")
        .in("id", sequenceIds)
    : { data: [] };

  const seqMap: Record<string, { template_key: string; subject: string | null }> = {};
  for (const s of seqRes.data ?? []) {
    seqMap[s.id] = { template_key: s.template_key, subject: s.subject };
  }
  ...
  const totalSpent = completedPurchases.reduce((s, p) => s + (p.amount_paid ?? p.amount ?? 0), 0);
        ...
        <header style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, flex: 1 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center" }}>
              <Chip bg="rgba(76,175,130,0.15)" color="#4CAF82">לקוח</Chip>
                    ...
                    {completedPurchases.map((p) => (
                      <div key={p.id} style={{
                        ...
                        <span style={{
                          ...
                          {"₪"}{Number(p.amount_paid ?? p.amount).toLocaleString("he-IL")}
                        </span>
            ...
            {emailLogs.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                {emailLogs.map((log: any) => {
                  const seq = seqMap[log.sequence_id] ?? null;
                  return (
                    <div key={log.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 0", borderBottom: "1px solid rgba(44,50,62,0.5)", flexWrap: "wrap" }}>
                      <span style={{ flex: 1, fontSize: 13, color: "#EDE9E1" }}>{seq?.subject ?? seq?.template_key ?? log.sequence_id}</span>
                      <span style={{
                        fontSize: 11, fontWeight: 700, padding: "2px 7px", borderRadius: 5,
                        background: log.status === "opened" ? "rgba(55,138,221,0.15)" : "rgba(76,175,130,0.15)",
                        color: log.status === "opened" ? "#378ADD" : "#4CAF82",
                      }}>
