  ...
  const message = await client.messages.create({
    ...
    tools: [{
      ...
      input_schema: {
        ...
        properties: {
          content_angle:    { type: "string", maxLength: 120, description: "משפט אחד: הפחד/חסם המרכזי לתוכן שיווקי" },
          watch_out:        { type: "string", maxLength: 120, description: "משפט אחד: אזהרה ספציפית מהנתונים" },
          headline:         { type: "string", maxLength: 120, description: "משפט אחד (עד 20 מילה) שמסכם את הממצא המרכזי" },
          audience_profile: { type: "string", maxLength: 200, description: "2-3 משפטים קצרים: מי הם, מה שלב העסק שלהם" },
          top_pains:        { type: "array", items: { type: "string", maxLength: 80 }, description: "3 כאבים — כל אחד עד 15 מילה" },
          product_fit:      { type: "string", maxLength: 200, description: "2-3 משפטים: התאמה בין המלצות לרצון ההשקעה" },
          opportunities:    { type: "array", items: { type: "string", maxLength: 80 }, description: "3 הזדמנויות — כל אחת עד 15 מילה" },
        },
        required: ["content_angle","watch_out","headline","audience_profile","top_pains","product_fit","opportunities"],
      },
    }],
