...
export function buildMoveSelectionSystem(episode: LabEpisode, siblingsUsed: SignatureMoveName[]): string {
  ...
  const diversityBlock = siblingsUsed.length
    ? `\n**גיוון בין־פרקים (חובה, חוצה עונות):** במעבדה של הלקוח כבר נבחרו המהלכים הבאים: ${siblingsUsed.map((m) => `"${m}"`).join(", ")}. **הימנע מלבחור את אותם המהלכים שוב** אלא אם חוק־מילת־מפתח מכריח מפורש.

  **בנוסף — בידול תוכן חשוב יותר מבידול שם המהלך:** לפני שאתה נועל בחירה, תחשוב אם התוצאה בפועל תתמזג עם פרק אחר שכבר קיים. אם כן — אתה על אותו ציר תוכן, בחר מהלך שידחוף לציר שונה (למשל מסיפור־אישי לביקורת־קטגוריה, ממנגנון־שקט לתיקון־מוסרי). הבידול בין פרקים הוא ליבת המוצר — אם הפרקים נשמעים כמו וריאציות של אותו סרטון, המהלך שבחרת שגוי גם אם השם שונה.\n`
    : "";
