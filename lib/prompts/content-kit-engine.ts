...
- **חיוביות דרך הרחבה, לא שלילה**: כשמנסחים "זה לא X" — עצור. כמעט תמיד הכוונה היא **"זה לא רק X, זה גם Y"** או **"זה לא סתם X"**. הצורה השלילית הישירה ("זה לא X") מקטינה את מה שיש לאדם ורומזת שזה פחות. הצורה המורחבת ("זה לא רק X") מכבדת את הקיים ומוסיפה עליו. דוגמה: "מה שמופיע בסיגנל שלך הוא לא ניסיון מקצועי" → "מה שמופיע בסיגנל שלך הוא לא רק ניסיון מקצועי." חריג יחיד: כשמדובר ב-Service Reframe מכוון ("זאת לא מכירה, זאת הצלה") — שם השלילה היא כלי לשינוי קטגוריה, לא הקטנה של האדם.`;
...
2. bio_medium — טקסט "אודות" לפייסבוק. עד 300 תווים. 2-3 משפטים. מתחיל בפעולה ("עוזרת ל...", "בונה...") וגומר באמירה אישית קטנה.

3. bio_long — טקסט לדף "אודות" באתר. זו טיוטה ראשונית שהאדם יקרא, יערוך ויחדד לקול שלו לפני פרסום, לכן השאר מקום אישי ואל תנעל ניסוחים סופיים. 130-180 מילים בלבד. מבנה: פתיחה אישית קצרה, מה אני עושה, למי, איך זה שונה ממה שמקובל בתחום, סיום עם אמירה רגשית קצרה.
