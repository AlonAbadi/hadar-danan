...
// 5000 tokens fits the full kit (bios ~600, manifesto ~700, persona ~250,
// strategy ~700, speaking topics ~500, 30 content ideas ~800, structural
// overhead ~400). Sonnet generates ~100 tok/s → ~50s, leaving 10s headroom
// under Vercel's 60s function timeout.
export const CONTENT_KIT_MAX_TOKENS = 5000;
...
3. bio_long — אבאוט לדף "אודות" באתר. 130-180 מילים (לא יותר). מבנה: פתיחה אישית קצרה → מה אני עושה → למי → איך זה שונה ממה שמקובל בתחום → סיום עם אמירה רגשית קצרה. בלי "אני נולדתי ב...". בלי קלישאות.
...
5. manifesto — האני מאמין של האדם. 300-450 מילים (לא יותר). מבנה: 4-5 פסקאות, כל אחת מתחילה ב"אני מאמין ש..." או "אני יודע ש..." או "אני לא מסכים ש...". הצהרות חדות שמושכות אליו את הקהל הנכון ודוחות את הקהל הלא נכון. סיום עם משפט אחד שמסכם את העמדה כולה. זה לא רכך, זו עמדה.
...
10. speaking_topics — בדיוק 5 פריטים. כל פריט הוא [כותרת הרצאה] שורת ריווח אחת [תקציר 35-55 מילים מה ההרצאה אומרת]. ההרצאות חייבות להיות נושאים שהאדם הוא הסמכות הטבעית להם, נגזרים מהבידול שלו, לא נושאים גנריים שכל אחד בתחום יכול להעביר.
