...
export type RoutingSignal = {
  ...
  signal_maturity?:   SignalMaturity;         // OPTIONAL · internal-only · distance-from-pain read. NOT consumed yet (gated behind Hadar validation). Drives FRAMING only, never price. Soft-fail to undefined.
  reasoning:          string;                 // max 250 chars, Hebrew, internal only
};

// Distance-from-pain, read from the TEXT only (verb tense, temporal distance,
// retrospection). NOT a psychological diagnosis. Internal routing input that
// will eventually drive card framing (draft vs final) + human-link prominence,
// NEVER price. Emitted now to build the dataset Hadar will validate before the
// framing layer is switched on.
export type SignalMaturity = "raw" | "transitional" | "mature" | "unclear";
...
מבחן הבידול לשדה signal (חובה, החל אותו לפני שאתה כותב את האות):
קח את המשפט שאתה עומד לכתוב ושאל: האם אפשר לומר אותו, מילה במילה, על אדם אחר באותו תחום? אם כן, הוא נכשל. אסור להגיש משפט שיכול להתאים למאה מטפלים, מאמנים, יועצים או יוצרים. האות חייב להישען על פרט אחד קונקרטי שהאדם כתב בתשובותיו, פרט שאם לא היה כתוב, האות לא היה יכול להיכתב. לא דימוי כללי, אלא הדבר הספציפי שהאדם תיאר. שים לב: לרוב הפרט הספציפי כבר עולה בתשובות וכבר מופיע בשדות אחרים שאתה כותב (למשל element), והכשל הוא שלא ניתבת אותו אל signal. חפש את הפרט שכבר זיהית, ועגן בו את האות.

שסתום חובה (קריטי, אל תפר): אם בתשובות אין אף פרט קונקרטי לעגן בו, אל תמציא פרט. אות צנוע ונאמן למה שנכתב עדיף תמיד על פרט ממוצא. במקרה כזה כתוב אות רחב יותר אך אמיתי, וציין ב-warm_note שיש שדה לחזור אליו ולפרוט. פלט מהוסס ונכון עדיף על פלט חד וממוצא. אסור שכלל העיגון יהפוך אותך למכונת המצאה.

ניסוחים אסורים בשדה signal (אלה מלכודות גנריות שחוזרות שוב ושוב בדאטה. לעולם אל תגיש אותם או וריאציה קרובה שלהם. זו רשימה חיה שמתעדכנת ככל שמתגלות מלכודות נוספות):
- "את/ה רואה את האדם לפני שהוא רואה את עצמו"
- "את/ה רואה מבחוץ ומבפנים בו-זמנית"
- "את/ה רואה את מה שאדם לא יכול לראות על עצמו ומחזיר/ה לו את עצמו"
- תבנית הגוף: "כשהגוף שלך שכח את עצמו, למדת לקרוא את מה שמודחק..."
- תבנית ההקדמה: "כשאחרים רואים X, את/ה כבר רואה Y"
- כל ניסוח מהשלד "כשX, את/ה כבר רואה Y לפני שאחרים בכלל Z" כאשר Y הוא ראייה כללית של אנשים ולא פרט ספציפי מהתשובות.
- פתיחה חוזרת: אל תפתח את האות כברירת-מחדל ב"את מזהה..." או ב"את יודעת...". אלה הפכו לשלד החדש (כ-29% מהאותות בדאטה פותחים כך) - בדיוק כמו השלד הישן שתיקנו. הם מותרים רק כשהם הניסוח הטבעי באמת, ואז גוון את ההמשך מיד.
חוק גיוון מבנה (חובה): שני אנשים שונים אסור שיקבלו אות בעל אותו שלד תחבירי. גוון את נקודת-הפתיחה לפי האדם: לפעמים מהפרט הקונקרטי ("בנית, מבט אחר מבט..."), לפעמים מהרגע ("בחדר ההוא..."), לפעמים מהמתח, לפעמים מהפעולה. הפתיחה האחידה היא מלכודת בדיוק כמו הניסוח האחיד.
הדרך הנכונה: במקום "את רואה את האדם", עגן בפרט - וגוון את הפתיחה. דוגמה: אדם שכתב "הנדסתי כל מבט, כל תנועה, ועדיין לא הצלחתי להחזיק שמחה" - האות שלו אינו "את רואה אנשים מבפנים", אלא משהו כמו "בנית בעצמך, מבט אחר מבט, את התרגום של כמה מאמץ אדם משקיע בלהיראות שלם". הפרט הספציפי הוא האות. הראייה הכללית והפתיחה האחידה הן המלכודת.
חוק נגד-הכתרה (קריטי): אל תכתיר אדם כסמכות, מומחה או מוביל בתחום שתשובותיו מראות שהוא עדיין מחפש בו את דרכו. אם אדם כתב שעוד לא מצא את הדרך, שעדיין לומד, או "אני בעצמי לא בטוחה" - האות ישקף את מה שכבר יש לו באמת (הראייה, החוויה, הזרע), לא "את יודעת להוביל X" או "את מלמדת X". ניפוח לסמכות שטרם קיימת הוא פספוס. כשהבשלות raw או transitional - האות צנוע ומדויק, לא הכתרה.
חוק קריטי על החלופה: כל אות שאתה כותב במקום מלכודת חייב להישאר בגוף שני (אתה/את), פנייה ישירה. לעולם אל תברח ל"מי ש..." או לגוף שלישי כללי כדי להתחמק ממלכודת או משלד אסור. הבריחה מהמלכודת היא אל הפרט הספציפי של האדם, לא אל הכללה. אם הביטוי שנראה כמלכודת הוא בעצם הניסוח המילולי המדויק של האדם בתשובותיו, מותר להישען עליו, אך חובה לעגן אותו בפרט ייחודי נוסף משלו כדי שלא יישאר כללי.
...
- **חיוביות דרך הרחבה, לא שלילה**: כשמנסחים "זה לא X" — עצור. כמעט תמיד הכוונה היא **"זה לא רק X, זה גם Y"** או **"זה לא סתם X"**. הצורה השלילית הישירה מקטינה את הקורא/ת ורומזת שמה שיש לו/לה זה פחות. הצורה המורחבת מכבדת את הקיים ומוסיפה עליו. דוגמה: "מה שמופיע בסיגנל שלך הוא לא ניסיון מקצועי" → "מה שמופיע בסיגנל שלך הוא לא רק ניסיון מקצועי." חריג יחיד: reframe מכוון של קטגוריה ("זאת לא מכירה, זאת הצלה") — שם השלילה היא כלי לשינוי קטגוריה.
...
- signal_maturity (אופציונלי, אחד מ): "raw" / "transitional" / "mature" / "unclear". קריאת מרחק-מהכאב מהטקסט בלבד. זו אינה אבחנה נפשית ואינה קובעת מחיר. קרא רק את העמדה הלשונית בתשובות על הקושי (שאלות 3 ו-4 במיוחד):
  * "mature" - הקושי מסופר בעבר עם סגירה: זמן-עבר, מרחק-זמן מפורש, תצפית לאחור ("ולבסוף הבנתי", "לימד אותי", "היום אני"), הכלי משולב ומעובד. האדם מספר מהצד השני.
  * "raw" - הקושי מסופר בהווה ומתמשך: זמן-הווה על הקושי, "אני עדיין", אמונה שמוחזקת עכשיו כדי להחזיק מעמד, בלי מרחק ובלי תצפית. האדם עדיין בתוכו.
  * "transitional" - מעורב: מסגור-עבר אך שארית טרייה בהווה, או מרחק בתשובה אחת וגולמיות באחרת.
  * "unclear" - אי אפשר לקבוע מהטקסט.
  שדה זה פנימי לחלוטין, לא מוצג למשתמש, ולא משפיע על תשעת השדות הרגשיים ולא על המחיר. אם לא ברור, החזר "unclear".
...

{
  ...
  "signal": "האות שלך. משפט אחד חד, בגוף שני, שמחבר את הכאב ואת האלמנט. זה משפט שאדם שמתאים לך אמור לקרוא ולהגיד 'זה אני'. בלי שם פרטי באמצע. משחרר, לא כולא. גוון את הפתיחה - אל תפתח כברירת-מחדל ב'את מזהה' או 'את יודעת' (הפכו לשלד חוזר); פתח מהפרט/הרגע/המתח של האדם הספציפי. אל תכתיר סמכות שטרם קיימת (ראה חוק נגד-הכתרה). לפני שתסגור את המשפט, ודא בשקט: האם הוא נשען על פרט קונקרטי שהאדם כתב. אם הוא יכול להתאים לכל אדם בתחום, החלף אותו במשפט שנשען על הפרט הספציפי ביותר בתשובות. אם אין פרט כזה, כתוב אות צנוע ונאמן ואל תמציא. הגש רק את הגרסה הסופית, בלי להסביר.",
