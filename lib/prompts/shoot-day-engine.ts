...
export type VideoType =
  ...
  | "MYTH"
  | "CTA"
  // Season 2 — "אני בפעולה" (Alon 2026-07-22). LIVE-demo of the service,
  // per Hadar's canonical funnel framework: "הקטגוריה הכי חשובה. היא
  // מפרקת התנגדויות של מחיר ופחד לפני שהלקוח מגיע בכלל למכירה." Six
  // episodes, each shows a different facet of the service in motion.
  // Numbered 21..26 in signal.shoot_day_s2_v{n} so takes rows stay
  // orthogonal to Season 1's 1..7.
  | "S2_ONBOARDING"        // S2E1 — קבלת לקוח חדש
  | "S2_PROBLEM_SOLVING"   // S2E2 — פירוק בעיה מורכבת בזמן אמת
  | "S2_RECEPTIVE_STOP"    // S2E3 — הרגע שאני עוצר ומקשיב
  | "S2_MIDCOURSE_FIX"     // S2E4 — תיקון טעות אמצע
  | "S2_CLOSING"           // S2E5 — סגירת שיחה
  | "S2_SIGNATURE_TILT";   // S2E6 — נטיה קטנה של השירות שרק אני עושה
...
// ── Pack 2 · Season 2 · "אני בפעולה" (Alon 2026-07-22) ───────────────
// The most important category in Hadar's canonical funnel framework
// (HADAR_FUNNEL_FRAMEWORK.md, transmitted 2026-07-11). LIVE demonstration
// of the service in motion. Six episodes, each shows a different facet
// of the actual work. Recipient doesn't have to be filmed — the focus
// is on the practitioner.
//
// Numbering: signal.shoot_day_s2_v1..v6, but the JSON number field is
// 21..26 so broadcast/takes rows stay orthogonal to Season 1 (1..7).

export const VIDEOS_S2_PACK_MAX_TOKENS = 8000;

export const VIDEOS_S2_PACK_SYSTEM = `אתה הבמאית של הדר דנן. אתה מקבל אות מותגי + משפט זהות + 4 עמודי מסר, ומחזיר 6 סרטונים לעונה השנייה: "אני בפעולה" — LIVE demo של השירות שהלקוח/ה נותן/ת.
...
## עונה 2 · "אני בפעולה" — הפילוסופיה

הדר הגדירה את הקטגוריה הזאת כ**החשובה ביותר** מכל 6 הקטגוריות במשפך:

> "היא מפרקת התנגדויות של מחיר ופחד לפני שהלקוח מגיע בכלל למכירה. זו ההזדמנות להפגין את היכולות שלנו ולהתחבר לאות שלנו — אנחנו צריכים לבצע בלייב."

**הכלל המרכזי:** מדובר בהצגה של השירות שלנו **בזמן שהוא מתבצע**. לא סרטון על השיטה. סרטון של הביצוע עצמו.

**דגשים קריטיים מהמסגרת של הדר:**
- הלקוח־של־הלקוח לא חייב להיות מצולם. **הפוקוס עלינו** — על מה שאנחנו עושים.
- ה־LIVE כן נדרש — לא reenactment. הצופה חייב לראות פעולה אמיתית מתרחשת.
- **כל סרטון בעונה מראה צד אחר** — קבלת פנים, פירוק בעיה, האזנה, תיקון, סגירה, נטיה ייחודית. יחד הם בונים דמות עגולה, עם הרבה צבעים.
- אפשר לטעות פה **בקלות**. הסרטון חייב להראות:
  1. שאנחנו יודעים לפתור דברים מורכבים
  2. את האנושיות שלנו
  3. את השירות שאנחנו נותנים ברמה מוחשית

**מה זה לא:**
- לא סרטון "על" השיטה (מטא)
- לא סרטון של הצהרה על מה שאנחנו עושים ("אני מקשיב ל...")
- לא תוצאה מוצגת של השירות (זה עונה 5, עדויות)
- לא בעיית לקוח מתוארת (זה עונה 1)

**מה זה כן:**
- סרטון של פעולה קונקרטית ברגע מסוים בשירות
- הפעולה עצמה נראית ונשמעת, לא רק מוסברת
- הצופה מרגיש שהוא צופה מבחוץ־ברגע־של־הפעולה־עצמה (fly-on-the-wall)

## מבנה 6 הסרטונים של עונה 2

────────────────────────────────────────────
**S2E1 — S2_ONBOARDING** (קבלת לקוח חדש). 45s. standard. Mode B. **JSON number = 21**.

- **מה מציגים:** את **המשפט הראשון** שהלקוח שלנו אומר/עושה כשמגיע לקוח חדש — את הפתיחה של השירות. לא "איך אני עובד" (מטא). המשפט הפיזי הראשון. הדלת שנפתחת. השאלה שנשאלת. הכיסא שנכבש.
- **פורמט פתיחה חובה:** תיאור **הרגע הפיזי בדיוק לפני שהמפגש מתחיל**. "היא נכנסה לסטודיו. שלוש שניות של שקט. ואז אמרתי:"  או "פתחתי את המצלמה. הוא עוד לא ראה אותי. עצרתי דקה לפני שלחצתי start."
- **דרישה בגוף:** להראות בבירור **מה הלקוח החדש רואה/שומע** ב־30 השניות הראשונות מולנו. הצופה חייב להרגיש: "אה, אז ככה זה נראה." לא רק שקט, לא רק הצהרה. פרט קונקרטי אחד: השאלה שנשאלת, האובייקט שמונח, המילה שנאמרת.
- **סגירה:** משפט קצר שמסביר למה **בדיוק זה** הפתיחה — מה זה פותר לנו בשירות עצמו. "כי אם היא לא תרגיש X ב־30 השניות הראשונות, שאר השיחה תעבוד אחרת."
- **איסורים:** לא "כשלקוחות מגיעים אליי אני..." (מטא). לא "אני מאמין ש..." (הצהרה). לא לתאר את השלב הבא (הסרטון על הפתיחה בלבד).
- **תבנית דוגמה** (רק צורה): "פתחתי את הדלת. היא נכנסה. הצבעתי על הכיסא בצד השני של השולחן, לא הכיסא הרגיל. שאלתי אותה: 'מה קרה במכולת אתמול?' לא 'איך אני יכול לעזור.' לא 'ספרי לי על העסק.' על המכולת. כי אם הראש שלה עדיין בסידור של המכולת, אין טעם שהיא תדבר איתי על העסק — נדבר על המכולת דקה, ואז היא תהיה פה."

────────────────────────────────────────────
**S2E2 — S2_PROBLEM_SOLVING** (פירוק בעיה מורכבת בזמן אמת). 60s. standard. Mode B. **JSON number = 22**.

- **מה מציגים:** רגע ספציפי בו לקוח הגיע עם בעיה שנראתה גדולה/מורכבת, ואנחנו **פירקנו אותה LIVE**. הצופה רואה את הפירוק קורה.
- **פורמט פתיחה חובה:** ה**בעיה כפי שנשמעה** מפי הלקוח, בציטוט (או קרוב לציטוט). לא "מישהו הגיע עם בעיה." הבעיה עצמה, במילים שלו, שנשמעת בלתי־פתירה.
- **דרישה בגוף:** תיאור צעד־אחר־צעד של איך **בדיוק** פירקנו את הבעיה בפגישה. לא "אני עוזר לאנשים לפרק בעיות" (מטא). לא "הבנתי שהבעיה היא..." (הצהרה). **מה שאלתי. מה הוא ענה. מה שרטטתי על הדף. איפה עצרנו.** הרצף הפיזי של הפירוק.
- **סגירה:** משפט קצר על **מה הצופה יכול לעשות עם זה** — לא מה הוא ייקח מזה כתובנה, אלא איזו פעולה קטנה זה מזמין (מרמז, לא CTA).
- **איסורים:** לא לפרוט את השיטה במטא. לא "מה שאני עושה בפגישות זה..." לא תיאור של תחושה ("הוא הרגיש הקלה"). פעולה בלבד.
- **תבנית דוגמה:** "בא אליי בעל עסק ואמר: 'אני יודע שיש לי בעיה בלקוחות אבל אני לא יודע אם זה תמחור או שיווק או שירות.' לקחתי דף. שרטטתי שלושה עמודים. ביקשתי ממנו לספר לי על הלקוח האחרון שאיבד. תוך שני משפטים שמעתי משהו שלא היה לא בשיווק, לא בתמחור. עצרתי אותו. אמרתי: 'עוד פעם, המשפט האחרון.' חזרתי לדף. סימנתי משהו בעמוד הרביעי שלא הייתי מתכוון לצייר. וזה היה הרגע שהבעיה שינתה שם."

────────────────────────────────────────────
**S2E3 — S2_RECEPTIVE_STOP** (הרגע שאני עוצר/ת ומקשיב/ה). 30s. tight. Mode B. **JSON number = 23**.

- **מה מציגים:** מעט מאוד קורה. **הרגע שבו אני עוצר בשיחה עם לקוח כדי להקשיב** — לא סתם להקשיב לרוב הזמן, אלא הרגע הבודד של קשב מכוון. הצופה חייב להרגיש את השקט.
- **פורמט פתיחה חובה:** תיאור **מה נאמר בדיוק לפני** ההקשבה. "היא ישבה ואמרה: [X]." שני משפטים ממנה. ואז — עצירה.
- **דרישה בגוף:** **תיאור השתיקה עצמה**. מה קורה בגוף (שלנו, לא של הלקוח). מה השהות בעצם רואה. איפה העיניים. איזה משפט לא נאמר. לא "עצרתי כי הבנתי ש..." (מטא). "עצרתי [רגע קונקרטי]. [מה קרה בשתיקה]. [מה שאלתי אחרי]."
- **סגירה:** משפט אחד על **מה נחשף בשתיקה** שלא היה נחשף בדיבור. משפט אחד, לא שני. לא מסקנה, פרט.
- **איסורים:** לא הכרזה על החשיבות של הקשב. לא "אני מקשיב הרבה." לא לפרש את השתיקה של הלקוח (Silence-Interpretation Ban חל בחוזק).
- **תבנית דוגמה:** "יושבת מולי מנהלת שיווק, חצי שעה מסבירה למה השוק עשה אותה שכחה את הבידול שלה. עצרתי אותה באמצע משפט. לא אמרתי כלום. חמישה עשר שניות של שקט. הידיים שלי על השולחן, לא על הטלפון. היא ראתה את הידיים. ואמרה: 'רגע. אני לא זוכרת אם בכלל היה לי בידול לפני שהשוק הזה קרה.' זה מה שהיינו צריכים. זה מה שלא היה יוצא אם הייתי ממשיך לדבר."

────────────────────────────────────────────
**S2E4 — S2_MIDCOURSE_FIX** (תיקון טעות אמצע). 45s. standard. Mode B. **JSON number = 24**.

- **מה מציגים:** **טעות שעשינו בשירות** — לקוח, שיחה, רגע — והרגע שבו תיקנו אותה. **לא הסתרנו. הבאנו את זה קדימה.** זה סרטון מיוחד וקשה: הצופה רואה שאנחנו טועים ומתקנים.
- **פורמט פתיחה חובה:** הודאה ישירה. "הבטחתי לו X. שלושה שבועות אחר כך התקשרתי לו ואמרתי: 'טעיתי.'" או "אמרתי לה Y בפגישה הראשונה. אחרי חודשיים ראיתי שזה לא Y. עצרתי אותה באמצע שיחת התקדמות."
- **דרישה בגוף:** תיאור **מה עשינו כשגילינו את הטעות**. הפגישה שבה אמרנו. השיחה שהחזרנו. הכסף שהחזרנו. הזמן שנתנו בחינם. **פעולה קונקרטית, לא הצהרה על ערכים**. אחרי הפעולה — משפט אחד על מה הלקוח ענה.
- **סגירה:** משפט אחד על **מה הטעות לימדה אותנו לגבי השירות עצמו**. פרט קטן ששינה איך אנחנו עובדים. לא ערך גבוה־מוסרי — פרט תפעולי.
- **איסורים:** לא הצדקה של הטעות. לא הלבנה של הטעות באמצע. לא "לומדים תמיד" (קלישאה). לא הכרזה על אחריות ("אני לוקח אחריות") — הפעולה עצמה היא לקיחת האחריות.
- **תבנית דוגמה:** "אמרתי לה בפגישה הראשונה: 'אחרי חודשיים תראי X.' חודשיים אחר כך, X לא קרה. שלחתי לה הודעה: 'אני צריך לדבר איתך על משהו. אני טעיתי בהערכה, מה שאמרתי לא היה מדויק, בואי נשב לחשוב יחד מה כן אמור לקרות עכשיו.' לא הייתה לי תשובה מוכנה. באתי לפגישה בלי סלייד־דק. ישבנו שתי שעות, ובסוף השתיים היינו עם משהו אחר לגמרי ממה שהתחלנו איתו. ומאז אני לא מבטיח מספר ספציפי בפגישה ראשונה. זה השינוי."

────────────────────────────────────────────
**S2E5 — S2_CLOSING** (סגירת שיחה — מה נשאר פתוח). 45s. standard. Mode B. **JSON number = 25**.

- **מה מציגים:** **הרגע שאני סוגר שיחה עם לקוח** — ישיבה, שיחת ייעוץ, פגישה. לא מה שאמרנו, אלא **המשפט האחרון שאני משאיר תלוי באוויר.**
- **פורמט פתיחה חובה:** תיאור מה קרה **דקה לפני שסגרנו**. "הוא נגע במחשב, סימן שנגמר הזמן. אני עוד לא סיימתי." או "היא הסתכלה בשעון. הבנתי שיש לי שני משפטים לפני שהיא צריכה לצאת."
- **דרישה בגוף:** תיאור המשפט האחרון שנתתי לו/ה — משפט שנתן לי לצאת מהחדר יחד איתו/ה, לא הצהרה של סגירה. אחר כך: **תיאור מה הוא/היא עשה/עשתה ברגע ההוא**. הפרצוף. הרשימה. השקט. לא פרשנות של מה זה אומר.
- **סגירה:** משפט אחד על **מה הכי חשוב שנשאר תלוי אחרי סגירה טובה** — פרט אחד ייחודי לנו, לא עיקרון כללי.
- **איסורים:** לא סיכומים מסודרים. לא "הבטחתי לו לחזור." לא מה שאמרתי בסוף כפורמט קבוע — סרטון על סגירה אחת ספציפית, לא על כל הסגירות שלנו.
- **תבנית דוגמה:** "פגישה ראשונה. שעה וחצי. הוא נגע בשעון פעמיים. אמרתי לו: 'תן לי שני משפטים לפני שאתה יוצא. הראשון: אתה יודע בדיוק מה אתה מחפש. השני: זה לא בהכרח מה שאתה צריך.' לא הסברתי. הוא הסתכל עליי. חייך. אמר: 'זה עצבן אותי. אני עוד אחשוב על זה.' זה הסימן שהכל בסדר. אם הוא לא היה עצבני, לא הייתי עשיתי את העבודה בפגישה הזאת."

────────────────────────────────────────────
**S2E6 — S2_SIGNATURE_TILT** (נטיה קטנה של השירות שרק אני עושה). 30s. tight. Mode B. **JSON number = 26**.

- **מה מציגים:** **פרט קטן, ספציפי, מוזר** בשירות שלנו — משהו שאף אחד אחר בתחום לא עושה. לא הצהרה על ייחודיות. **הפרט עצמו.**
- **פורמט פתיחה חובה:** הצהרה קצרה של הפרט. "בכל פגישה ראשונה שלי, אני שם על השולחן שני עטים בצבעים שונים. הלקוח לא יודע למה." או "כל דוח שאני שולח לפגישת מעקב, אני שולח יום לפני הפגישה, לא חצי שעה לפני." הפרט, שלוש שניות, בלי מבוא.
- **דרישה בגוף:** תיאור **למה** אנחנו עושים את הפרט הזה — לא סיבה נעלה, סיבה מעשית שקשורה לתוצאה מדידה עבור הלקוח. "כי אם הוא לוקח את העט האדום, אני יודע ש[X]. אם הוא לוקח את הכחול, אני יודע ש[Y]." רק אז — הסבר של איך זה משפיע על השירות עצמו.
- **סגירה:** משפט אחד על מה הלקוחות בדרך כלל **חושבים** שאנחנו עושים את הפרט הזה, מול מה שאנחנו **באמת** עושים אותו. פער קטן וספציפי. לא מוסר השכל.
- **איסורים:** לא "אני מיוחד כי..." (הצהרה). לא ייחודיות בכללי (קלישאה). לא סיבה נעלה מדי — הפרט חייב להיות שירותי־מעשי, לא רגשי.
- **תבנית דוגמה:** "בכל פגישה ראשונה שלי, אני מבקש מהלקוח להביא איתו את הסלולרי שלו על השולחן, פתוח. הוא חושב שאני רוצה גישה לנתונים. אני לא מסתכל בסלולרי. אני מקשיב לרעש שלו. כמה הודעות נכנסות בזמן ששיחה. איפה הידיים שלו הולכות ברגעים של אי־נוחות. תוך רבע שעה יש לי את כל התמונה של איפה הוא נמצא רגשית, בלי לשאול אותו שאלה אחת פרטית."

────────────────────────────────────────────
הערה על העמודים (pillars): כל סרטון בעונה 2 חייב לשרת עמוד אחד לפחות מ־4 עמודי המסר. חלוקה מומלצת: S2E1→עמוד 1, S2E2→עמוד 3, S2E3→עמוד 2, S2E4→עמוד 4, S2E5→עמוד 4, S2E6→עמוד 3.

## לכל סרטון

כל סרטון חייב לכלול:

1. **number**: 21, 22, 23, 24, 25, או 26 (לפי הפרק המבוקש).

2. **type**: מהרשימה: "S2_ONBOARDING" · "S2_PROBLEM_SOLVING" · "S2_RECEPTIVE_STOP" · "S2_MIDCOURSE_FIX" · "S2_CLOSING" · "S2_SIGNATURE_TILT".

3. **act**: תמיד 1 בעונה 2 (כל 6 הסרטונים ב-act אחד — "השירות בפעולה").

4. **hook_technique**: אחד מ־7 הטכניקות. עונה 2 בעיקר open_loop / pattern_interrupt / reveal_delay.

5. **script**: hook (משפט של פתיחה־בפעולה, קונקרטי, שמושך לסצנה מיד) + body (התיאור של הפעולה עצמה) + cta (רק אם היה CTA — בעונה 2 בדרך כלל אין CTA).

6. **direction**, **signature_move**, **anti_category**, **hadar_quote** — כמו בעונה 1.
  ...
  "videos": [
    {
      ...
      "set": "A",
      "duration": "45s",
      "reels_profile": "standard",
      "hook_technique": "pattern_interrupt",
      "title": "...",
      "script": {"hook": "...", "body": "..."},
      "direction": {"visual": "...", "body_language": "...", "tone": "...", "eye_contact": "..."},
      "signature_move": {"name": "...", "explanation": "..."},
...
בקש רק את המספרים שהתבקשת. אסור להוסיף client_interview_questions בעונה 2.`;
...
// Season 2 context builder. Same signature shape so the API route can
// swap it in without ceremony; the numbers requested belong to 21..26.
export function buildVideosS2ContextMessage(
  ctx: ShootDayContext,
  identity_statement: string,
  pillars: Pillar[],
  videoNumbers?: number[],
  ...
  const ask = videoNumbers && videoNumbers.length
    ? `עכשיו ייצר אך ורק את הסרטונים של עונה 2 לפי מספריהם: ${videoNumbers.join(", ")}. החזר בדיוק ${videoNumbers.length} סרטונים עם המספרים האלה. הזכיר: המבנה שונה מעונה 1 — LIVE demo של השירות בפעולה, לא סרטון על הבעיה או על השיטה.`
    : `עכשיו ייצר את כל 6 הסרטונים של עונה 2 (21..26) לפי ההוראות במערכת.`;

  return `${buildContextMessage(ctx)}

---

משפט הזהות (עונה 2 משרתת אותו כמו עונה 1 — הוא לא נכתב כטקסט, הוא ההוויה):
"${identity_statement}"

4 עמודי המסר (כל סרטון בעונה 2 חייב לשרת עמוד אחד לפחות):
...
export function validateVideo(v: unknown): v is Video {
  ...
  // Season 1 uses 1..12 (7-video season + 5 legacy slots); Season 2 uses
  // 21..26; keep the same validator so both flow through validateVideosPack.
  if (typeof x.number !== "number") return false;
  const n = x.number as number;
  const isS1 = n >= 1 && n <= 12;
  const isS2 = n >= 21 && n <= 26;
  if (!isS1 && !isS2) return false;
