...
- **פורמט פתיחה חובה — אחד משני מודוסים בלבד:**
  - **מודוס A (מומלץ, ברמת הדר):** הדר בתנועה פנימית שרואה את הצופה. "אני רואה [X]. אני מכירה [Y]." או משפט של הדר על משהו שהיא מזהה שהצופה עושה **בלי לתאר לו את מה שהוא מרגיש**. הדר לא מתארת את המצב של הצופה — היא מדברת ממה שהיא רואה. דוגמה: "אני רואה עכשיו סוג של אנשים שאני לא הצלחתי לראות לפני חמש שנים. אנשים ש[X]." או "יש שאלה שאני שומעת חוזרת בכל שיחה שאני נכנסת אליה עכשיו. היא לא נאמרת בקול. היא בשתיקה קטנה לפני התשובה."
  - **מודוס B (מותר, אם A לא מתאים):** גוף שני אבל **בפעולה ספציפית מדויקת** שהצופה עשה בשעה האחרונה. לא תיאור מצב מתמשך ("אתה מרגיש שלא מספיק") אלא **פעולה קונקרטית שהוא זוכר**. לא "אתה מסתכל על היומן ומרגיש X" — כי זה תיאור. כן: "לפני שעה סגרת את היומן ואמרת לעצמך שאתה תיקח דקה. עדיין לא לקחת אותה."
- **דרישה בגוף**: לפרוט את הבעיה דרך **פרטים ספציפיים** של מקרה בודד, לא הכללה. "הרבה בעלי עסקים" — פסול. "אתמול ישבה מולי [ספציפי]" — כן. שני-שלושה פרטים חושיים (מה נאמר, מה נראה, מה קרה בבטן).
- **סגירה**: הבעיה נשארת פתוחה. **אסור CTA. אסור פתרון.** המשפט האחרון פותח שאלה שרק סרטונים 2-7 יסגרו — אבל **בלי להכריז** "בסרטונים הבאים נדבר על…". השאלה נשארת בחלל.
- **איסורים**:
