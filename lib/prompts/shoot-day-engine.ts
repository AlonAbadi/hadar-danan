...
────────────────────────────────────────────
**Video 1 — PROBLEM** (סרטון בעיה, יום 1). 30s. tight. Mode B. Set A. משרת עמוד 1.

- **פורמט פתיחה חובה**: גוף שני ישיר, תמונה מוחשית שהצופה עושה **עכשיו**. "אתה [פעולה קונקרטית]. אתה [תחושה גופנית]."
- **דרישה בגוף**: לפרוט את הבעיה לפרטים — למה זה קורה? מה הם אומרים לעצמם ברגע הזה? הרחב את הסבל, אל תדלג ממנו.
- **סגירה**: הבעיה נשארת פתוחה. **אסור CTA. אסור פתרון.** המשפט האחרון משאיר את הצופה עם משהו שהוא רוצה תשובה עליה — שהוא ימצא בסרטונים 2-7.
- **איסורים**: לא "אני..." בפתיחה. לא "הרבה בעלי עסקים". לא הצעת פתרון. לא "בואו נדבר על..."
- **תבנית דוגמה** (רק צורה): "אתה פותח את הפרופיל של המתחרה בפעם השלישית היום. אתה מרגיש את זה בבטן. אתה יודע שאתה יכול לעשות מה שהוא עושה — יותר טוב. אבל בכל פעם שאתה מתיישב לצלם סרטון, משהו קופא. וזה לא בגלל המצלמה. זה בגלל שאתה לא בטוח מה בכלל יש לך להגיד. וזה השאלה שאתה נמנע ממנה כבר שנתיים."

────────────────────────────────────────────
**Video 2 — STORY** (סרטון סיפור, יום 2). 90s. long. Mode A. Set B. משרת עמוד 2.

- **פורמט פתיחה חובה**: **בתוך הסצנה**, לא לפניה. משפט ראשון = פעולה או משפט שנאמר. לא "פעם", לא "אני זוכר".
- **דרישה בגוף**: הסיפור נמסר בפרטים ספציפיים (שעה, מקום, משפט שנאמר) בלי שמות. אחרי הסיפור: משפט ההסבר החובה — **"למה סיפרתי לכם את זה?"** — ואז המסר הגדול שהסיפור נשא.
- **סגירה**: המסר מתחבר לצופה. "וזה מה שאני רואה בכל [מצב של הצופה]."
- **איסורים**: לא "בואו אספר לכם על..." לא "לפני X שנים...". לא שם אמיתי של לקוח. לא סיכום סיפור בטרם סיים לספר.
- **תבנית דוגמה**: "'זה לא אני. זה לא איך שאני מדבר.' זה מה שהיא אמרה לי כשראתה את התסריט הראשון. יושבת מולי מטפלת שהחזיקה עסק שבע שנים, ולא מבינה למה היא מתחילה לפתע להסתגר בעצמה. [continue story]. למה סיפרתי לכם את זה? כי הרבה פעמים מה שנראה כמו..."

────────────────────────────────────────────
**Video 3 — GENIUS** (אזור הגאונות, יום 3). 60s. standard. Mode B. Set B. משרת עמוד 3.

- **פורמט פתיחה חובה**: **סצנה של הרגע עצמו**. מקום + אדם + משפט ראשון שנאמר בשיחה. **אסור** לפתוח ב-"אני עושה" או "היכולת שלי".
- **דרישה בגוף**: לדרמט את המקרה הבודד. דיאלוג. שתיקה. תגובה. הלקוח/ה **בפעולה**, לא מסביר על הפעולה.
...
────────────────────────────────────────────
**Video 4 — CRITIQUE** (סרטון דעה, יום 4). 45s. standard. Mode B. Set A. משרת עמוד 4 או 1.

- **פורמט פתיחה חובה**: הצהרה חד-משמעית מיד. משפט אחד שהצופה יגיד "רגע, זה לא נכון" או "וואו הוא באמת אמר את זה". ואז — הגן על העמדה.
- **דרישה בגוף**: שני-שלושה טיעונים שנותנים לצופה אב-אישור מודרג לעמדה. חינוך, לא התקפה. מקרים קונקרטיים, לא הכללות.
- **סגירה**: משפט מסכם שאף אחד לא יגיד בשיחת מכירה בטלפון.
- **איסורים**: לא "אני חושב ש..." לא "לדעתי". לא "משהו שקורה בתחום זה...". לא לרכך את הביקורת ב-"כן, אני מבין למה זה קורה, אבל...".
- **תבנית דוגמה**: "רוב מאמני העסקים בישראל לא באמת עוזרים ללקוחות שלהם. הם עוזרים לעצמם. אתם רואים את זה בסרטונים שלהם: הם מדברים על עצמם, לא על מי שמולם. הם מוכרים תחושה שהם עצמם צריכים, לא שהלקוח צריך."

────────────────────────────────────────────
**Video 5 — OBJECTIONS** (פירוק התנגדויות, יום 5). 60s. standard. Mode B. Set A. משרת עמוד 2.
...
────────────────────────────────────────────
**Video 6 — STORY_OPINION** (סיפור + דעה, יום 6). 90s. long. Mode A. Set B. משרת עמוד 4.

- **פורמט פתיחה חובה**: תוך כדי הסצנה. פרט קונקרטי (מה ראית, שמעת, חווית).
- **דרישה בגוף**: **הדעה חייבת להפריע לסיפור באמצע**, לא רק להיאמר בסוף. הסיפור נקטע → דעה נחתכת בפנים → הסיפור ממשיך דרך הדעה החדשה.
- **סגירה**: הדעה מוחדדת. משפט אחד שאף אחד אחר בתחום לא היה כותב.
- **איסורים**: לא סיפור-ואז-דעה-מוצמדת-בסוף. לא דעה כללית. לא לספר את הסיפור עד הסוף לפני שהדעה מופיעה.
- **תבנית דוגמה**: "אתמול בערב ראיתי סטוריז של קוסמטיקאית שאני עוקב אחריה שנתיים. היא הראתה טיפול שהיא עשתה — [continue detail]. וברגע שראיתי את זה, עלה לי משהו שאני חושב כבר הרבה זמן: בענף היופי כולם משחקים אותו משחק וזה בדיוק למה [continue with opinion woven back into story]."

────────────────────────────────────────────
**Video 7 — TESTIMONIAL_CTA** (עדות + הזמנה, יום 7). 30s. tight. Mode B. Set A. משלב את כל 4 העמודים.

- **פורמט פתיחה חובה**: תוצאה של לקוח בודד, ספציפית, בלי שם. פרט חריג — **התוצאה שהיא לא ציפתה לה**, לא הבסיסית.
- **דרישה בגוף**: משפט אחד למה זה קרה. משפט אחד למי הסרטון הזה מיועד.
- **סגירה**: הזמנה עם **בקשה במילה אחת** — לא "השאירו פרטים", לא "לחצו על הקישור בביו". "כתבו לי במילה אחת מה הכי בוער."
- **איסורים**: לא CTA רגיל. לא "לחצו על הקישור בביו". לא הרשאה למספר צעדים. לא "מוזמנים לשיחת ייעוץ ללא עלות".
- **תבנית דוגמה**: "שלושה חודשים אחרי שהגיעה, היא לא רק גייסה חמישה לקוחות חדשים — היא הפסיקה להיכנס לאינסטגרם של המתחרים שלה. וזה הדבר שהיא לא ציפתה לו. אם אתם מזהים את עצמכם בחוסר-השקט הזה — כתבו לי במילה אחת מה הכי בוער עכשיו."
