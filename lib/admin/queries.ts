...
export async function getGA4Data(dateRange?: string) {
  ...
  try {
    ...
    const [summaryRes, channelsRes, eventsRes] = await Promise.all([
      ...
      fetch(`${baseUrl}:runReport`, {
        ...
        body: JSON.stringify({
          ...
          dimensionFilter: {
            filter: { fieldName: 'eventName', inListFilter: { values: ['purchase', 'generate_lead', 'begin_checkout', 'sign_up'] } },
          },
    ...
    const [summary, channels, events] = await Promise.all([summaryRes.json(), channelsRes.json(), eventsRes.json()]);
