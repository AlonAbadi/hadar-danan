...
// ─── GA4 Data API ─────────────────────────────────────
// NOTE: google-auth-library cannot be bundled client-side (native Node modules).
// GA4 integration is disabled — use a dedicated server-only API route if needed.
export async function getGA4Data(_dateRange?: string) {
  return { configured: false, data: null };
}
